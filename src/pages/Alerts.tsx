...
export function Alerts({ alerts, onUpdateAlerts }: AlertsProps) {
  ...
  useEffect(() => {
    ...
    if (!isAdmin && user?.vehicleId) {
      result = result.filter(alert => alert.vehicleId === user.vehicleId);
    ...
    setFilteredAlerts(result);
  }, [alerts, searchTerm, statusFilter, vehicleFilter, isAdmin, user]);

  const handleAddAlert = () => {
    const targetVehicleId = isAdmin ? newAlert.vehicleId : user?.vehicleId;
    if (!newAlert.descricao || !targetVehicleId || !newAlert.dataExpiracao) {
      return;
    }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        {(isAdmin || (!!user?.vehicleId)) && (
          <Button onClick={() => {
            if (!isAdmin && user?.vehicleId) {
              setNewAlert({ ...newAlert, vehicleId: user.vehicleId });
            }
            setShowNewAlertForm(!showNewAlertForm);
          }}>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {isAdmin ? (
                ...
                <Input value={user?.vehicleId ? getVehicleInfo(user.vehicleId) : ""} readOnly />
              )}
              <Select 
                value={newAlert.tipo} 
