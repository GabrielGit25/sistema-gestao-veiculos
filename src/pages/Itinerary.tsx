...
export function Itinerary({ vehicles }: ItineraryProps) {
  ...
  const handleDeleteItinerary = (group: ItineraryPhoto[]) => {
    const itineraryId = group[0]?.itineraryId;
    if (itineraryId) {
      setItineraryPhotos(prev => prev.filter(p => p.itineraryId !== itineraryId));
    } else {
      const idsToRemove = new Set(group.map(g => g.id));
      setItineraryPhotos(prev => prev.filter(p => !idsToRemove.has(p.id)));
    }
  };

  const handleAddEmptyItinerary = () => {
    const itineraryId = Date.now();
    const baseVehicleId = user?.vehicleId || 0;
    const baseDriverId = user?.driverId || 0;
    const baseDriverName = user?.username || "Exemplo";
    const nextId = Math.max(0, ...itineraryPhotos.map(p => p.id)) + 1;
    const newStart: ItineraryPhoto = {
      id: nextId,
    ...
    setItineraryPhotos(prev => [newStart, newEnd, ...prev]);
  };
  ...
  return (
    <div className="max-w-full overflow-x-hidden">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold">Quilometragem</h1>
          <p className="text-muted-foreground">Histórico de Quilometragem do Veículo</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" onClick={handleAddEmptyItinerary} title="Novo registro">
            ...
            {filteredPhotos.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">
                  {isDriver 
                    ? "Nenhum registro. Adicione a foto de partida para iniciar"
                    : "Nenhum motorista enviou fotos do painel ainda"
                  }
                </p>
                {isDriver && (
                  <Button className="mt-3" onClick={() => {
                    const newId = Math.max(0, ...itineraryPhotos.map(p => p.id)) + 1;
                    const newItineraryId = Date.now();
                    ...
                    <Plus className="w-3 h-3 mr-1" /> Adicionar Partida
                  </Button>
                )}
              </div>
            ) : (
              groupedPhotos.map((photoGroup, groupIndex) => (
                 <div key={groupIndex} className="space-y-4 pb-6 border-b last:border-b-0">
                  {/* Nome do motorista, data e botão de exclusão do registro */}
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Button
                      ...
                      onClick={() => handleDeleteItinerary(photoGroup)}
                    >
                      <X className="w-4 h-4" />
                    </Button>
                  </div>
                  ...
                  <div className="flex flex-col md:flex-row items-start gap-6">
                    {(() => {
                      const startPhoto = photoGroup.find(p => p.position === 'start' && p.photoUrl) ?? photoGroup.find(p => p.position === 'start') ?? photoGroup[0] ?? null;
                      const endPhoto = photoGroup.find(p => p.position === 'end' && p.photoUrl) ?? (photoGroup.find(p => p.position === 'end') ?? (photoGroup.length > 1 ? photoGroup[1] : null));
                      return (
                        <>
                          <div className="flex flex-col items-center">
                            <div className="text-xs text-muted-foreground mb-2">Partida</div>
                            {startPhoto && startPhoto.photoUrl ? (
                              <>
                                <img
                              ...
                              <div className="w-24 h-24 rounded-lg border-2 border-dashed border-border flex flex-col items-center justify-center text-xs text-muted-foreground">
                                ...
                                <Button className="h-7 mt-2 px-2 text-xs" onClick={() => {
                                  if (startPhoto) {
                                    setEditTarget(startPhoto);
                                    setEditDialogOpen(true);
                                    return;
                                  }
                                  const base = photoGroup.find(p => p.position === 'end') ?? photoGroup[0];
                                  if (!base) return;
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="border-2 border-dashed rounded-lg p-4 text-center cursor-pointer" onClick={() => editFileInputRef.current?.click()}>
              ...
              <input id="itinerary-edit-file" ref={editFileInputRef} type="file" accept="image/*" className="hidden" onChange={handleEditModalFileSelect} />
            </div>
