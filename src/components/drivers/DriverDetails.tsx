...
export function DriverDetails({ driver, onBack, onUpdate, onDelete }: DriverDetailsProps) {
  ...
  const isDriverBlank = (d: Driver) => {
    const placeholderNome = d.nome === 'Novo Motorista';
    const emptyCpf = !d.cpf || d.cpf.trim() === '';
    const emptyCnh = !d.cnh || d.cnh.trim() === '';
    return placeholderNome || emptyCpf || emptyCnh;
  };

  const handleBack = () => {
    if (isAdmin && isDriverBlank(editedDriver)) {
      const confirmed = window.confirm('Há campos em branco neste motorista. Deseja sair mesmo assim?');
      if (!confirmed) return;
      onDelete(editedDriver.id);
    }
    onBack();
  };
