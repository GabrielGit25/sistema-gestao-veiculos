...
export function VehicleDetails({ vehicle, onBack, onUpdate, onDelete }: VehicleDetailsProps) {
  ...
  const [driverHistory, setDriverHistory] = useState<Array<{ id: number; driverName: string; date: string; note?: string }>>([]);
  ...
  const handleDriverChange = (driverName: string) => {
    ...
    const entry = { id: Date.now(), driverName, date: new Date().toISOString() };
    setDriverHistory(prev => [entry, ...prev]);
  };
  ...
  useEffect(() => {
    if (vehicle.motorista && !driverHistory.length) {
      setDriverHistory([{ id: Date.now(), driverName: vehicle.motorista, date: new Date().toISOString() }]);
    }
  }, [vehicle]);
  ...
  const isVehicleBlank = (v: Vehicle) => {
    const emptyModelo = !v.modelo || v.modelo.trim() === '';
    const placeholderPlaca = v.placa?.toUpperCase().startsWith('NOVA');
    const noMotorista = !v.motorista || v.motorista === '-';
    return emptyModelo || placeholderPlaca || noMotorista;
  };

  const handleBack = () => {
    if (isAdmin && isVehicleBlank(editedVehicle)) {
      const confirmed = window.confirm('Há campos em branco neste veículo. Deseja sair mesmo assim?');
      if (!confirmed) return;
      onDelete(editedVehicle.id);
    }
    onBack();
  };
    ...
    <div>
      <div className="bg-card rounded-lg shadow-sm p-6 mb-6">
        ...
        <div className="flex gap-6 flex-col lg:flex-row">
          {/* Vehicle Image */}
          <div 
            className="relative w-48 h-32 rounded-lg overflow-hidden bg-secondary shrink-0 group"
            onClick={isAdmin ? () => setShowImageModal(true) : undefined}
          >
        ...
        <div className="mt-6">
          ...
          {activeTab === 'historico' && (
            <div className="bg-card rounded-lg p-6">
              <h2 className="text-lg font-semibold mb-6">Histórico de Motoristas</h2>
              {isAdmin && (
                <div className="flex items-center gap-2 mb-4">
                  <Button
                    onClick={() => {
                      const name = prompt('Nome do motorista');
                      if (!name) return;
                      const note = prompt('Observação (opcional)') || undefined;
                      const entry = { id: Date.now(), driverName: name, date: new Date().toISOString(), note };
                      setDriverHistory(prev => [entry, ...prev]);
                    }}
                  >
                    <Plus className="w-4 h-4 mr-2" />
                      ...
                      <div key={entry.id} className="flex items-center justify-between border rounded-md p-3">
                        ...
                        {isAdmin && (
                          <Button
                            variant="destructive"
                            size="sm"
                            onClick={() => setDriverHistory(prev => prev.filter(e => e.id !== entry.id))}
                          >
                            Excluir
                          </Button>
                        )}
